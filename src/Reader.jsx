    ...
    <div style={{
      ...
      <div style={{
        ...
        <button
          ...
          style={{
            ...
            background: 'oklch(0 0 0 / 0.52)', color: '#fff',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            opacity: 0, transition: 'opacity 180ms ease', fontSize: 14,
          }}
      ...
      {editing ? (
        <input
          ...
          onKeyDown={e => { if (e.key === 'Enter' || e.key === 'Escape') commitCaption(); }}
          placeholder="Add a title…"
          style={{
            fontFamily: 'var(--serif)', fontSize: 14, fontStyle: 'italic',
            color: 'var(--ink-soft)', caretColor: 'var(--accent)',
            textAlign: 'center', borderBottom: '1px solid var(--accent-soft)', paddingBottom: 3,
          }}
        />
      ) : (
        <button
...
function PhotoSection({ photos, onPhotosChange, phase }) {
  ...
  function handleFiles(files) {
    ...
    toProcess.forEach(file => {
      ...
      reader.onload = e => {
        onPhotosChange(prev => [
          ...prev,
          { id: 'p-' + Math.random().toString(36).slice(2, 8), dataUrl: e.target.result, caption: '' },
        ]);
      };
  ...
  function updateCaption(id, caption) { onPhotosChange(prev => prev.map(p => p.id === id ? { ...p, caption } : p)); }
  function deletePhoto(id) { onPhotosChange(prev => prev.filter(p => p.id !== id)); }
    ...
    <div style={{
      ...
      {isEmpty ? (
        <div
          ...
          onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--accent-soft)'; e.currentTarget.style.background = 'oklch(0 0 0 / 0.02)'; }}
        ...
        >
          ...
          <span style={{ fontFamily: 'var(--sans)', fontSize: 11, letterSpacing: '0.12em', textTransform: 'uppercase', color: 'var(--ink-ghost)' }}>
            Add photos
          </span>
        </div>
      ) : (
        <div style={{
          ...
          {photos.map((photo, i) => (
            <PhotoCard
              ...
              onUpdateCaption={updateCaption} onDelete={deletePhoto}
            />
          ))}
          <button
            onClick={() => fileInputRef.current?.click()}
            style={{
              flexShrink: 0, width: 80, aspectRatio: '1', borderRadius: 14,
              border: '1.5px dashed var(--paper-edge)',
              display: 'flex', flexDirection: 'column', alignItems: 'center',
              justifyContent: 'center', gap: 6, cursor: 'pointer',
              color: 'var(--ink-ghost)', transition: 'border-color 200ms, color 200ms',
              alignSelf: 'flex-start',
            }}
            onMouseEnter={e => { e.currentTarget.style.borderColor = 'var(--accent-soft)'; e.currentTarget.style.color = 'var(--ink-soft)'; }}
            onMouseLeave={e => { e.currentTarget.style.borderColor = 'var(--paper-edge)'; e.currentTarget.style.color = 'var(--ink-ghost)'; }}
          >
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none">
              <path d="M8 3v10M3 8h10" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/>
            </svg>
            <span style={{ fontFamily: 'var(--sans)', fontSize: 9, letterSpacing: '0.10em', textTransform: 'uppercase' }}>Add</span>
          </button>
        </div>
      )}

      <input ref={fileInputRef} type="file" accept="image/*" multiple onChange={handleInputChange} style={{ display: 'none' }} />
...
export default function Reader({ entry, fromRect, onClose, onSave }) {
  ...
  // Refs so close() always reads the latest values without stale closure issues
  ...
  function commit() {
    const trimTitle = latestTitle.current.trim() || 'Untitled';
    const newBody   = latestBody.current.split(/\n\n+/).map(s => s.trim()).filter(Boolean);
    onSave && onSave({
      ...entry,
      title: trimTitle,
      body: newBody.length ? newBody : [''],
      photos: latestPhotos.current,
    });
  }
      ...
      >
        ...
        >
          ...
          >
            ...
            {/* Body — editable, looks like paragraphs */}
            <textarea
              value={body}
